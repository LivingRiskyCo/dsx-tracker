...
with st.sidebar:
    ...
    try:
        ...
        st.sidebar.error(f"⚠️ Tagging unavailable: {str(e)[:50]}")
    except Exception as e:
        TAGGING_AVAILABLE = False
        st.sidebar.error(f"⚠️ Tagging error: {str(e)[:50]}")
...
if page == "🎯 What's Next":
...
elif page == "🎥 Video Analysis Viewer":
    st.title("🎥 Video Analysis Viewer")
    ...
    if has_video and has_metadata:
        ...
        if overlay_data:
            ...
            # Create video player with overlays
        ...
        with st.expander("📋 How to Use"):
            st.markdown("""
            **Video Analysis Viewer** allows you to view recorded game videos with player tracking overlays.
            
            **Steps:**
            1. **Upload Video**: Either upload an MP4 file or provide a video URL
               - For large videos (>50MB), use a video URL (Google Drive, Dropbox, etc.)
               - For small videos, you can upload directly
            
            2. **Upload Metadata**: Upload the `*_overlay_metadata.json` file from your analysis
               - This file contains player positions, ball tracking, and trajectories
               - Generated automatically when you run analysis
            
            3. **View Overlays**: Toggle options in the sidebar to show/hide:
               - Player bounding boxes and names
               - Ball position
               - Player trajectories
               - Raw YOLO detection boxes
            
            **Tips:**
            - Works best on desktop browsers for full features
            - Mobile browsers support basic playback
            - Use video URLs for large files to avoid upload limits
            - Frame information shows detailed data for any frame number
            """)
    ...
    **Quick Access:**
    ...
    # 1. Extracted Matches Statistics
    try:
        if os.path.exists('Opponents_of_Opponents_Matches_Expanded.csv'):
            extracted_df = pd.read_csv('Opponents_of_Opponents_Matches_Expanded.csv', index_col=False)
            total_matches = len(extracted_df)
            matches_with_scores = len(extracted_df[(extracted_df['GF'].notna()) & (extracted_df['GA'].notna())])
            unique_teams = len(set(extracted_df['Team'].dropna().unique()) | set(extracted_df['Opponent'].dropna().unique()))
        else:
            total_matches = 0
    ...
    # 2. Division Data Statistics
    try:
        all_divisions_df = load_division_data()
        division_teams = len(all_divisions_df) if not all_divisions_df.empty else 0
        division_files_count = len([f for f in [
        ...
        division_teams = 0
        division_files_count = 0
    
    # 3. DSX Match Statistics
    try:
        if os.path.exists('DSX_Matches_Fall2025.csv'):
            dsx_matches = pd.read_csv('DSX_Matches_Fall2025.csv', index_col=False)
            dsx_total_games = len(dsx_matches)
            dsx_wins = len(dsx_matches[dsx_matches['Result'] == 'W']) if 'Result' in dsx_matches.columns else 0
            dsx_losses = len(dsx_matches[dsx_matches['Result'] == 'L']) if 'Result' in dsx_matches.columns else 0
            dsx_draws = len(dsx_matches[dsx_matches['Result'] == 'D']) if 'Result' in dsx_matches.columns else 0
            dsx_unique_opponents = len(dsx_matches['Opponent'].dropna().unique()) if 'Opponent' in dsx_matches.columns else 0
        else:
            dsx_total_games = 0
            dsx_wins = 0
            dsx_losses = 0
            dsx_draws = 0
            dsx_unique_opponents = 0
    except:
        dsx_total_games = 0
        dsx_wins = 0
        dsx_losses = 0
    ...
    # 4. Discovered Tournaments Statistics
    try:
        if os.path.exists('Ohio_Tournaments_2018_Boys_Discovered_20251102.csv'):
            discovered_df = pd.read_csv('Ohio_Tournaments_2018_Boys_Discovered_20251102.csv', index_col=False)
            discovered_teams = len(discovered_df)
            discovered_tournaments = len(discovered_df['Tournament'].dropna().unique()) if 'Tournament' in discovered_df.columns else 0
        else:
            discovered_teams = 0
    ...
    # 5. Rankings Statistics
    try:
        rankings_2018_3plus = 0
        rankings_2018_6plus = 0
        rankings_comprehensive = 0
        
        if os.path.exists('Rankings_2018_Teams_3Plus_Games.csv'):
            rankings_2018_3plus_df = pd.read_csv('Rankings_2018_Teams_3Plus_Games.csv', index_col=False)
            rankings_2018_3plus = len(rankings_2018_3plus_df)
        
        if os.path.exists('Rankings_2018_Teams_6Plus_Games.csv'):
            rankings_2018_6plus_df = pd.read_csv('Rankings_2018_Teams_6Plus_Games.csv', index_col=False)
            rankings_2018_6plus = len(rankings_2018_6plus_df)
        
        if os.path.exists('Comprehensive_All_Teams_Rankings.csv'):
            comprehensive_df = pd.read_csv('Comprehensive_All_Teams_Rankings.csv', index_col=False)
            rankings_comprehensive = len(comprehensive_df)
    except:
        rankings_2018_3plus = 0
        rankings_2018_6plus = 0
        rankings_comprehensive = 0
    ...
    with col1:
        ...
        st.metric("Total Matches Tracked", f"{total_matches:,}")
        st.metric("Matches with Scores", f"{matches_with_scores:,}")
        st.metric("Score Coverage", f"{(matches_with_scores/total_matches*100) if total_matches > 0 else 0:.1f}%")
        st.metric("Unique Teams in Matches", f"{unique_teams:,}")
    
    with col2:
        st.subheader("🏆 Division & Rankings")
        st.metric("Division Files Loaded", division_files_count)
        st.metric("Teams in Divisions", f"{division_teams:,}")
        st.metric("Ranked Teams (3+ games)", rankings_2018_3plus)
        st.metric("Ranked Teams (6+ games)", rankings_2018_6plus)
        st.metric("Total Ranked Teams", rankings_comprehensive)
    
    with col3:
        st.subheader("⚽ DSX & Tournaments")
        st.metric("DSX Games Played", dsx_total_games)
        st.metric("DSX Record", f"{dsx_wins}-{dsx_losses}-{dsx_draws}")
        st.metric("DSX Opponents Faced", dsx_unique_opponents)
        st.metric("Tournaments Discovered", discovered_tournaments)
        st.metric("Teams Discovered", f"{discovered_teams:,}")
    ...
    with st.expander("📋 Detailed Statistics", expanded=False):
        ...
        with col_detail1:
            st.markdown("### Match Data Quality")
            if total_matches > 0:
                st.write(f"- **Total Matches**: {total_matches:,}")
                st.write(f"- **With Scores**: {matches_with_scores:,} ({(matches_with_scores/total_matches*100):.1f}%)")
                st.write(f"- **Without Scores**: {total_matches - matches_with_scores:,} ({((total_matches - matches_with_scores)/total_matches*100):.1f}%)")
            
            st.markdown("### DSX Performance")
            if dsx_total_games > 0:
                win_pct = (dsx_wins / dsx_total_games * 100) if dsx_total_games > 0 else 0
                st.write(f"- **Total Games**: {dsx_total_games}")
                st.write(f"- **Wins**: {dsx_wins} ({win_pct:.1f}%)")
                st.write(f"- **Losses**: {dsx_losses}")
                st.write(f"- **Draws**: {dsx_draws}")
                st.write(f"- **Unique Opponents**: {dsx_unique_opponents}")
        
        with col_detail2:
            st.markdown("### Data Coverage")
            st.write(f"- **Division Files**: {division_files_count} loaded")
            st.write(f"- **Division Teams**: {division_teams:,}")
            st.write(f"- **Discovered Tournaments**: {discovered_tournaments}")
            st.write(f"- **Discovered Teams**: {discovered_teams:,}")
            
            st.markdown("### Rankings Coverage")
            st.write(f"- **3+ Games Threshold**: {rankings_2018_3plus} teams")
            st.write(f"- **6+ Games Threshold**: {rankings_2018_6plus} teams")
            st.write(f"- **Comprehensive Rankings**: {rankings_comprehensive} teams")
            
            # Calculate coverage percentage
            if discovered_teams > 0:
                coverage_pct = (rankings_2018_3plus / discovered_teams * 100) if discovered_teams > 0 else 0
                st.write(f"- **Rankings Coverage**: {coverage_pct:.1f}% of discovered teams")
