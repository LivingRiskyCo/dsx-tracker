...
if page == "🏆 Division Rankings":
    ...
    try:
        ...
        # Comparison
        st.subheader("⚖️ DSX vs " + selected_opp)
        st.info(f"Comparing DSX (independent) vs {selected_opp} (OCL division team)")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("### 🟧 Dublin DSX")
            st.write(f"**Status:** Independent Team")
            st.write(f"**Comparative Rank:** #{int(dsx_row['Rank'])} (if in division)")
            st.write(f"**Strength Index:** {dsx_row['StrengthIndex']:.1f}")
            st.write(f"**PPG:** {dsx_row['PPG']:.2f}")
            st.write(f"**GF/Game:** {dsx_row['GF']:.2f}")
            st.write(f"**GA/Game:** {dsx_row['GA']:.2f}")
            st.write(f"**GD/Game:** {dsx_row['GD']:+.2f}")
        
        with col2:
            st.markdown(f"### {selected_opp.split()[-2] if len(selected_opp.split()) > 2 else selected_opp}")
            st.write(f"**Status:** OCL BU08 Division Team")
            st.write(f"**Division Rank:** #{int(opp_row['Rank'])} of 7")
            st.write(f"**Strength Index:** {opp_row['StrengthIndex']:.1f}")
            st.write(f"**PPG:** {opp_row['PPG']:.2f}")
            st.write(f"**GF/Game:** {opp_row['GF']:.2f}")
            st.write(f"**GA/Game:** {opp_row['GA']:.2f}")
            st.write(f"**GD/Game:** {opp_row['GD']:+.2f}")
        ...
        si_diff = dsx_row['StrengthIndex'] - opp_row['StrengthIndex']
        
        if si_diff > 10:
            st.success(f"✅ DSX is stronger (SI: +{si_diff:.1f})")
        elif si_diff < -10:
            st.error(f"⚠️ {selected_opp} is stronger (SI: {si_diff:.1f})")
        else:
            st.info(f"⚖️ Evenly matched (SI diff: {si_diff:+.1f})")
        
        # Offensive/Defensive comparison
        if dsx_row['GF'] > opp_row['GF']:
            st.write(f"⚽ **Offensive Advantage:** DSX scores {dsx_row['GF'] - opp_row['GF']:.2f} more goals/game")
        elif dsx_row['GF'] < opp_row['GF']:
            st.write(f"⚽ **Offensive Disadvantage:** {selected_opp} scores {opp_row['GF'] - dsx_row['GF']:.2f} more goals/game")
        
        if dsx_row['GA'] < opp_row['GA']:
            st.write(f"🛡️ **Defensive Advantage:** DSX allows {opp_row['GA'] - dsx_row['GA']:.2f} fewer goals/game")
        elif dsx_row['GA'] > opp_row['GA']:
            st.write(f"🛡️ **Defensive Disadvantage:** DSX allows {dsx_row['GA'] - opp_row['GA']:.2f} more goals/game")
        
        # Game plan suggestion
        st.markdown("---")
        st.subheader("📋 Suggested Game Plan")
        
        if opp_row['StrengthIndex'] > dsx_row['StrengthIndex'] + 10:
            st.write("**Defensive Focus:**")
            st.write("- Stay organized defensively")
            st.write("- Look for counter-attack opportunities")
            st.write("- Keep energy high for full 60 minutes")
            st.write("- A draw is a good result")
        elif opp_row['StrengthIndex'] < dsx_row['StrengthIndex'] - 10:
            st.write("**Offensive Focus:**")
            st.write("- Press high and dominate possession")
            st.write("- Create multiple scoring chances")
            st.write("- Build confidence for tougher matches")
            st.write("- Target: 3 points (win)")
        else:
            ...
            st.write("- Target: 1-3 points (draw or win)")
    else:
        st.error("Could not load division data")
